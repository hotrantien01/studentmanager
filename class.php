							...
							<tbody>
								...
								<tr>
									...
									<td class="text-center">
										<button class="btn btn-sm btn-primary edit_class" type="button" data-id="<?php echo $row['id'] ?>"  data-course_id="<?php echo $row['course_id'] ?>"  data-level="<?php echo $row['level'] ?>" data-section="<?php echo $row['section'] ?>" >Sửa</button>
										<button class="btn btn-sm btn-danger delete_class" type="button" data-id="<?php echo $row['id'] ?>">Xóa</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
	...
	$('#manage-class').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				...
				$('#msg').html('<div class="alert alert-danger mx-2">Lớp học đã tồn tại.</div>')
				end_load()
				}				
			}
		})
	...
	$('.delete_class').click(function(){
		_conf("Bạn có chắc chắn xóa lớp học này không?","delete_class",[$(this).attr('data-id')])
	})
