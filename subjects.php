					...
					<div class="card-footer">
						<div class="row">
							<div class="col-md-12">
								...
								<button class="btn btn-sm btn-default col-sm-3" type="reset"> Xóa</button>
							</div>
						</div>
					</div>
				</div>
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-center">
										<button class="btn btn-sm btn-primary edit_subject" type="button" data-id="<?php echo $row['id'] ?>"  data-subject="<?php echo $row['subject'] ?>"  data-description="<?php echo $row['description'] ?>" >Sửa</button>
										<button class="btn btn-sm btn-danger delete_subject" type="button" data-id="<?php echo $row['id'] ?>">Xóa</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
	...
	$('#manage-subject').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				...
				$('#msg').html('<div class="alert alert-danger mx-2">Môn học đã tồn tại.</div>')
				end_load()
				}				
			}
		})
	...
	$('.delete_subject').click(function(){
		_conf("Bạn có chắc chắn xóa môn học này không?","delete_subject",[$(this).attr('data-id')])
	})
